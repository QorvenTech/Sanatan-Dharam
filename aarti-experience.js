(function () {
  ...
  window.renderAartiReader = function (readerId) {
    ...
    return '<article class="aarti-reader" data-aarti-id="' + id + '" data-font-scale="1" style="--aarti-progress:0%;--aarti-line:1.42"><nav class="aarti-breadcrumb" aria-label="Breadcrumb"><button type="button" onclick="aartiReturnToLibrary()">' + tr('Home','होम') + '</button><span>/</span><button type="button" onclick="aartiReturnToLibrary()">' + tr('Aarti Sangrah','आरती संग्रह') + '</button><span>/</span><strong>' + (lang()==='hi'?d.titleHi:d.title) + '</strong></nav>' +
      ...
      '<section class="aarti-deity-card"><h2>' + tr('Associated Deity','संबद्ध देवता') + '</h2><div>' + (d.image?'<img src="'+d.image+'" alt="" width="55" height="55" loading="lazy" decoding="async">':'<span>ॐ</span>') + '<span><strong>' + (lang()==='hi'?d.deityHi:d.deity) + '</strong><small>' + tr(d.summary,d.summaryHi) + '</small></span></div><button type="button" onclick="aartiOpenDeity()">' + tr('Explore deity','देवता परिचय') + ' →</button></section><section class="aarti-variation-card"><h2>' + tr('Variation Note','पाठांतर टिप्पणी') + '</h2><p>' + tr('Widely recited versions differ in wording, verse order and local custom.','प्रचलित संस्करणों में शब्द, पद-क्रम और स्थानीय रीति का अंतर मिलता है।') + '</p></section></aside>' +
